<script lang="ts">
    ...
    onMount(async () => {
        ...
        for (const guild of user_data?.guilds ?? []) if (map.has(guild)) output.push([guild, map.get(guild) as string]);
    ...
    function check_ids() {
        ...
        for (const id of list)
            if (!id.match(/^[1-9][0-9]{16,19}$/))
                return (error = `Invalid ID: <code>${escape(id)}</code> is not a valid Discord ID.`);
    ...
    function submit(e: KeyboardEvent) {
        if (e.key === "Enter" && e.ctrlKey) select<HTMLFormElement>("form").requestSubmit();
    }
</script>
    ...
    <div class="container">
        <div id="main">
            ...
            {#if !servers?.length}
                <Callout style="red">
                    <p>
                        You are not staff on any TCN servers. If you believe this is a mistake, contact your server's
                        owner or a TCN observer.
                    </p>
                </Callout>
            {:else}
                {#if form?.error}
                ...
                <form method="post">
                    <div class="panel">
                        ...
                        {#if ids.trim() !== ""}
                            <div>
                                <br />
                                <a class="button" href={"javascript:void(0)"} on:click={check_ids}> Check IDs </a> (shows
                                the users' tags if all IDs are valid)
                            </div>
                        {/if}
                        <h4>Reason</h4>
                        <p>
                            If you need more than 498 characters, you should probably be putting it into the evidence
                            field instead.
                        </p>
                        <Textarea
                            name="reason"
                        ...
                        <h4>Evidence</h4>
                        <p>
                            For images, use Imgur or a similar service. Discord hosted image links work too, but keep in
                            mind that deleting the message containing the image will eventually cause the image link to
                            break.
                        </p>
                        <p>
                            If you need more than 1000 characters, please create a document and link it here. Include
                            some basic information in the evidence so people can see roughly what your document
                            contains.
                        </p>
                        <p>
                            Please spoiler anything suggestive and break up any image links to explicit / triggering
                            content so people can't accidentally access them.
                        </p>
                        <Textarea
                            name="evidence"
                    ...
                    <div class="panel">
                        ...
                        {:else}
                            <select name="server" required>
                                <option selected={servers.length > 1} disabled hidden value="">Select Server</option>
                    ...
                    <div class="panel">
                        <h4>Severity</h4>
                        <p>
                            The severity is used to determine auto-banning. A lower number indicates a greater threat.
                            Read the <a href="/info/banshares#severity">info page</a> for more information.
                        </p>
                    ...
                    <div class="panel">
                        <h4>Urgency</h4>
                        <p>
                            Check the box below to instruct the bot to ping all observers instead of just a few to
                            review this.
                        </p>
                        <label>
                    ...
                    <div class="panel">
                        ...
                        <br />
                        <p>
                            <b>WARNING:</b> You should only submit without checking IDs if your ID list is a link instead
                            of an actual list of IDs. Doing this will also prevent automatic banning. Consider just pasting
                            your entire ID list instead if possible - the bot will collapse it into a document link if it's
                            too long.
                        </p>
                        <p>
                            <b>This may take some time</b> if you have submitted a long ID list, as the bot needs time to
                            fetch all of the users.
                        </p>
                        <p>
                            If the list is very long and you want to get the banshare published ASAP, use "Submit
                            Without Validation", which will check if the IDs look correct but won't fetch all of the
                            users.
                        </p>
                    </div>
                </form>

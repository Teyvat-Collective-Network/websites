<script lang="ts">
    ...
    const feature_list = [
        ...
        ["[#channel]", "<span class='mention'><i class='material-icons'>tag</i> &nbsp; channel</span>"],
        [
        ...
        ["[:2020-05-27]", "<span class='mention'><i class='material-icons'>schedule</i> &nbsp; 2023-05-27</span>"],
        [
            "[reader]",
...
<div class="container">
    <div id="main">
        <Navigation last={["/info/documentation", "Website Documentation"]}>
            ...
            <p>
                The TCN Docs feature allows you to create a markdown document that renders as a webpage. You can control
                access based on the reader's TCN Council position as well as insert some Discord-related components such
                as user mentions.
            </p>
            <p>
            ...
            <p>
                To create a new document, click "New Document" on the docs home page. You can also edit documents that
                you have edit access for if you have the edit link (<code>/forms/edit/:ID</code>).
            </p>
            <Linkable id="content" e="h2" value="Content" />
            <p>
                The first section is the name and the content, and both are required. Write out the document content in
                markdown (this is the same formatting system Discord uses). Here is a (nearly comprehensive) list of
                Markdown features:
            </p>
                ...
                <div class="panel" style="width: max-content">
                    <h3 class="row" style="gap: 10px">
                        ...
                        <a href={"javascript:void(0)"} on:click={() => (show_markdown = !show_markdown)}>
                            <Icon icon={show_markdown ? "expand_less" : "expand_more"} />
                        </a>
                    </h3>
                    {#if show_markdown}
                        <table>
                                ...
                                <tr>
                                    <td style="padding: 20px">
                                        {#each list as item, index}
                                            {#if item.includes("<br />")}
                                                <pre style="background-color: var(--background-1-5)">{@html item}</pre>
                                            {:else}
                                                {#if index !== 0}&nbsp;/&nbsp;{/if}<code
                                                    style="background-color: var(--background-1-5)">{item}</code
                                                >
                                            {/if}
                                        {/each}
                                    </td>
                ...
                <div class="panel" style="width: max-content">
                    <h3 class="row" style="gap: 10px">
                        ...
                        <a href={"javascript:void(0)"} on:click={() => (show_features = !show_features)}>
                            <Icon icon={show_features ? "expand_less" : "expand_more"} />
                        </a>
                    </h3>
                    {#if show_features}
                        <table>
            ...
            <ul>
                ...
                <li>
                    Allow Observers to Edit - observers will also be able to edit the document but not access
                    permissions
                </li>
                <li>
                    Allow TCN Council to Edit - allow all TCN Council Members to edit the document but not access
                    permissions
                </li>
            ...
            <p>
                Control the document's appearance here. You can set it to be anonymous to hide your name from the
                document, but observers will still be able to see who owns the document.
            </p>
            <p>
                Observers can designate a document as official, which modifies the notice at the top of the document.
                Usually, it informs the reader that the document is user-generated content and not endorsed by the TCN,
                but an official document as designated by the observer team. Use this responsibly!
            </p>

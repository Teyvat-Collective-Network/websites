...
export default async function (interaction: ButtonInteraction, message: string): Promise<Banshare | void> {
    ...
    if (!post) {
        await interaction.reply({ content: "This message is missing from the database.", ephemeral: true });
    ...
    if (!banshare) {
        await interaction.reply({ content: "This banshare is missing from the database.", ephemeral: true });

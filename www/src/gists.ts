...
export async function create_gist(filename: string, description: string, content: string) {
    const request = await octokit.request("POST /gists", { description, files: { [filename]: { content } } });

...
export default function(fastify, opts, done) {
  fastify.get('/', async (request, reply) => {
    if (!request.query.code) return reply.redirect(
      `https://discord.com/oauth2/authorize?response_type=code&client_id=${process.env.DISCORD_ID}&scope=identify&redirect_uri=${process.env.DISCORD_REDIRECT}` 
  ...
  fastify.get('/token', async (request, reply) => {
    const user = await request.auth(true);
    if (!user) return reply.code(401).send();
  ...
  fastify.get('/user', async (request, reply) => {
    const user = await request.auth(true);
    return reply.code(user ? 200 : 400).send(user);
  });
